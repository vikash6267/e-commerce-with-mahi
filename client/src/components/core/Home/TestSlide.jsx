...
function TestSlide({ products }) {
  ...
  return (
    <div className='w-full overflow-x-auto  mt-6' style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch', scrollbarWidth: 'none', '-ms-overflow-style': 'none' }}>

      <div className='flex flex-nowrap justify-start px-6 '>
        {displayedProducts.map((product) => (
          
          <Link  to={`/product/${product._id}`} key={product._id} className='w-64 mx-2 flex flex-col '>
            <div className='h-full w-36 overflow-hidden relative lg:w-60 md:w-60' data-aos="fade-right" >
            <div>
            ...
            <div className=' '>
              <p className='font-montserrat text-[13px] text-gray-600'>{product.title}</p>
              <p className='font-montserrat text-[12px] text-gray-600 -mt-3'> {displayMoney(product.price)}</p>
